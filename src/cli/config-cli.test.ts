...
import type { ConfigFileSnapshot, ApmClawConfig } from "../config/types.js";
...
const mockWriteConfigFile = vi.fn<
  (cfg: ApmClawConfig, options?: { unsetPaths?: string[][] }) => Promise<void>
>(async () => {});

vi.mock("../config/config.js", () => ({
  readConfigFileSnapshot: () => mockReadConfigFileSnapshot(),
  writeConfigFile: (cfg: ApmClawConfig, options?: { unsetPaths?: string[][] }) =>
...
function buildSnapshot(params: {
  resolved: ApmClawConfig;
  config: ApmClawConfig;
}): ConfigFileSnapshot {
  return {
    path: "/tmp/apmclaw.json",
    exists: true,
...
function setSnapshot(resolved: ApmClawConfig, config: ApmClawConfig) {
...
function withRuntimeDefaults(resolved: ApmClawConfig): ApmClawConfig {
  return {
    ...resolved,
    agents: {
      ...resolved.agents,
...
function makeInvalidSnapshot(params: {
  ...
  return {
    path: params.path ?? "/tmp/custom-apmclaw.json",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: {},
...
describe("config cli", () => {
  ...
  describe("config set - issue #6070", () => {
    it("preserves existing config keys when setting a new value", async () => {
      ...
      const runtimeMerged: ApmClawConfig = {
  ...
  describe("config get", () => {
    it("redacts sensitive values", async () => {
      ...
      expect(mockLog).toHaveBeenCalledWith("__APMCLAW_REDACTED__");
    });
  });
  ...
  describe("config validate", () => {
    ...
    it("returns machine-readable JSON with --json for invalid config", async () => {
      ...
      expect(payload.path).toBe("/tmp/custom-apmclaw.json");
  ...
  describe("config set parsing flags", () => {
    it("falls back to raw string when parsing fails and strict mode is off", async () => {
      const resolved: ApmClawConfig = { gateway: { port: 18789 } };
      setSnapshot(resolved, resolved);
  ...
  describe("config file", () => {
    it("prints the active config file path", async () => {
      ...
      expect(mockLog).toHaveBeenCalledWith("/tmp/apmclaw.json");
      expect(mockWriteConfigFile).not.toHaveBeenCalled();
    });
    ...
    it("handles config file path with home directory", async () => {
      ...
      snapshot.path = "/home/user/.apmclaw/apmclaw.json";
      mockReadConfigFileSnapshot.mockResolvedValueOnce(snapshot);

      await runConfigCommand(["config", "file"]);

      expect(mockLog).toHaveBeenCalledWith("/home/user/.apmclaw/apmclaw.json");
    });
  });
});
