...
describe("resolveApiKeyForProfile fallback to main agent", () => {
  const envSnapshot = captureEnv(["APMCLAW_STATE_DIR", "APMCLAW_AGENT_DIR", "PI_CODING_AGENT_DIR"]);
  let tmpDir: string;
  ...
  beforeEach(async () => {
    ...
    // Set environment variables so resolveApmClawAgentDir() returns mainAgentDir
    process.env.APMCLAW_STATE_DIR = tmpDir;
    process.env.APMCLAW_AGENT_DIR = mainAgentDir;
