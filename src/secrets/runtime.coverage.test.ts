...
function buildConfigForApmClawTarget(entry: SecretRegistryEntry, envId: string): ApmClawConfig {
  const config = {} as ApmClawConfig;
  const refTargetPath =
...
describe("secrets runtime target coverage", () => {
  ...
  it("handles every apmclaw.json registry target when configured as active", async () => {
    const entries = listSecretTargetRegistryEntries().filter(
      (entry) => entry.configFile === "apmclaw.json",
    );
    for (const [index, entry] of entries.entries()) {
      const envId = `APMCLAW_SECRET_TARGET_${index}`;
      const expectedValue = `resolved-${entry.id}`;
      const snapshot = await prepareSecretsRuntimeSnapshot({
        config: buildConfigForApmClawTarget(entry, envId),
  ...
  it("handles every auth-profiles registry target", async () => {
    ...
    for (const [index, entry] of entries.entries()) {
      const envId = `APMCLAW_AUTH_SECRET_TARGET_${index}`;
      const expectedValue = `resolved-${entry.id}`;
      const snapshot = await prepareSecretsRuntimeSnapshot({
        config: {} as ApmClawConfig,
        env: { [envId]: expectedValue },
        agentDirs: ["/tmp/openclaw-agent-main"],
