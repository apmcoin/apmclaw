...
async function resolveContext(config: ApmClawConfig, sessionKey: string, workspaceDir: string) {
...
function createWorkSetupCommandConfig(scope: "agent" | "shared"): ApmClawConfig {
  return {
    agents: {
      defaults: {
        sandbox: {
      ...
      agents: {
        defaults: {
          sandbox: {
            mode: "all",
            scope: "agent",
            workspaceRoot: "~/.apmclaw/sandboxes",
          },
        },
        list: [
          {
