...
import { buildWebSearchProviderConfig, withTempHome, writeApmClawConfig } from "./test-helpers.js";
import { ApmClawSchema } from "./zod-schema.js";

describe("$schema key in config (#14998)", () => {
  it("accepts config with $schema string", () => {
    const result = ApmClawSchema.safeParse({
  ...
  it("rejects non-string $schema", () => {
    const result = ApmClawSchema.safeParse({ $schema: 123 });
...
describe("cron webhook schema", () => {
  it("accepts cron.webhookToken and legacy cron.webhook", () => {
    const res = ApmClawSchema.safeParse({
      cron: {
        enabled: true,
...
describe("config strict validation", () => {
  ...
  it("does not mark resolved-only gateway.bind aliases as auto-migratable legacy", async () => {
    await withTempHome(async (home) => {
      await writeApmClawConfig(home, {
        gateway: { bind: "${APMCLAW_BIND}" },
      });

      const prev = process.env.APMCLAW_BIND;
      process.env.APMCLAW_BIND = "0.0.0.0";
      try {
        const snap = await readConfigFileSnapshot();
        expect(snap.valid).toBe(false);
        ...
        if (prev === undefined) {
          delete process.env.APMCLAW_BIND;
        } else {
          process.env.APMCLAW_BIND = prev;
        }
      }
    });
  });
