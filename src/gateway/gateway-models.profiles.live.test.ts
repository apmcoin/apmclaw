...
import { resolveApmClawAgentDir } from "../agents/agent-paths.js";
...
import { ensureApmClawModelsJson } from "../agents/models-config.js";
import { isRateLimitErrorMessage } from "../agents/pi-embedded-helpers/errors.js";
import { discoverAuthStorage, discoverModels } from "../agents/pi-model-discovery.js";
import { loadConfig } from "../config/config.js";
import type { ModelsConfig, ApmClawConfig, ModelProviderConfig } from "../config/types.js";
...
const LIVE = isTruthyEnvValue(process.env.LIVE) || isTruthyEnvValue(process.env.APMCLAW_LIVE_TEST);
const GATEWAY_LIVE = isTruthyEnvValue(process.env.APMCLAW_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.APMCLAW_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.APMCLAW_LIVE_GATEWAY_PROVIDERS);
...
const GATEWAY_LIVE_PROBE_TIMEOUT_MS = Math.max(
  30_000,
  toInt(process.env.APMCLAW_LIVE_GATEWAY_STEP_TIMEOUT_MS, 90_000),
);
...
function resolveGatewayLiveMaxModels(): number {
  const gatewayMax = toInt(process.env.APMCLAW_LIVE_GATEWAY_MAX_MODELS, -1);
  if (gatewayMax >= 0) {
    return gatewayMax;
  }
  // Reuse shared live-model cap when gateway-specific cap is not provided.
  return Math.max(0, toInt(process.env.APMCLAW_LIVE_MAX_MODELS, 0));
}
...
function sanitizeAuthConfig(params: {
  ...
  let profiles: NonNullable<ApmClawConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    configPath: process.env.APMCLAW_CONFIG_PATH,
    token: process.env.APMCLAW_GATEWAY_TOKEN,
    skipChannels: process.env.APMCLAW_SKIP_CHANNELS,
    skipGmail: process.env.APMCLAW_SKIP_GMAIL_WATCHER,
    skipCron: process.env.APMCLAW_SKIP_CRON,
    skipCanvas: process.env.APMCLAW_SKIP_CANVAS_HOST,
    agentDir: process.env.APMCLAW_AGENT_DIR,
  ...
  process.env.APMCLAW_SKIP_CHANNELS = "1";
  process.env.APMCLAW_SKIP_GMAIL_WATCHER = "1";
  process.env.APMCLAW_SKIP_CRON = "1";
  process.env.APMCLAW_SKIP_CANVAS_HOST = "1";

  const token = `test-${randomUUID()}`;
  process.env.APMCLAW_GATEWAY_TOKEN = token;
  const agentId = "dev";

  const hostAgentDir = resolveApmClawAgentDir();
  ...
  process.env.APMCLAW_STATE_DIR = tempStateDir;
  ...
  process.env.APMCLAW_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.apmclaw-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveApmClawAgentDir();
  const sanitizedCfg: ApmClawConfig = {
    ...params.cfg,
  ...
  const tempConfigPath = path.join(tempDir, "apmclaw.json");
  await fs.writeFile(tempConfigPath, `${JSON.stringify(nextCfg, null, 2)}\n`);
  process.env.APMCLAW_CONFIG_PATH = tempConfigPath;
  ...
  try {
            ...
            const toolProbe = await withGatewayLiveProbeTimeout(
              client.request<AgentFinalPayload>(
                "agent",
                {
                  ...
                  message: strictReply
                    ? "ApmClaw live tool probe (local, safe): " +
    ...
    process.env.APMCLAW_CONFIG_PATH = previous.configPath;
    process.env.APMCLAW_GATEWAY_TOKEN = previous.token;
    process.env.APMCLAW_SKIP_CHANNELS = previous.skipChannels;
    process.env.APMCLAW_SKIP_GMAIL_WATCHER = previous.skipGmail;
    process.env.APMCLAW_SKIP_CRON = previous.skipCron;
    process.env.APMCLAW_SKIP_CANVAS_HOST = previous.skipCanvas;
    process.env.APMCLAW_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.APMCLAW_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      await ensureApmClawModelsJson(cfg);
      ...
      const rawModels = process.env.APMCLAW_LIVE_GATEWAY_MODELS?.trim();
      ...
      if (selectedCandidates.length < candidates.length) {
        logProgress(
          `[all-models] capped to ${selectedCandidates.length}/${candidates.length} via APMCLAW_LIVE_GATEWAY_MAX_MODELS=${maxModels}`,
        );
      }
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.apmclaw-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
