...
function buildParams(commandBody: string, cfg: ApmClawConfig, ctxOverrides?: Partial<MsgContext>) {
...
describe("handleCommands /allowlist", () => {
  ...
  it("removes DM allowlist entries from canonical allowFrom and deletes legacy dm.allowFrom", async () => {
    ...
    for (const testCase of cases) {
      ...
      const written = writeConfigFileMock.mock.calls.at(-1)?.[0] as ApmClawConfig;
...
describe("handleCommands ACP-bound /new and /reset", () => {
  ...
  const buildDiscordBoundConfig = (): ApmClawConfig =>
    ({
      commands: { text: true },
      bindings: [
        {
