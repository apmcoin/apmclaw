...
function listAgents(cfg: ApmClawConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: ApmClawConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<ApmClawConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: ApmClawConfig): AgentLookupCache {
...
export function pickFirstExistingAgentId(cfg: ApmClawConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: ApmClawConfig["bindings"];
...
const evaluatedBindingsCacheByCfg = new WeakMap<ApmClawConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  ApmClawConfig,
  {
    bindingsRef: ApmClawConfig["bindings"];
    agentsRef: ApmClawConfig["agents"];
    sessionRef: ApmClawConfig["session"];
...
function resolveRouteCacheForConfig(cfg: ApmClawConfig): Map<string, ResolvedAgentRoute> {
