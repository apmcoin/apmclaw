...
async function resolveAllowedTmpMediaPath(params: {
  ...
  const apmClawTmpDir = path.resolve(resolvePreferredApmClawTmpDir());
  if (!isPathInside(apmClawTmpDir, resolved)) {
    return undefined;
  }
  await assertNoTmpAliasEscape({ filePath: resolved, tmpRoot: apmClawTmpDir });
  return resolved;
}
