...
import type { ApmClawConfig, ConfigFileSnapshot } from "../config/types.apmclaw.js";
...
vi.mock("../infra/openclaw-root.js", () => ({
  resolveApmClawPackageRoot: vi.fn(),
}));
...
const { resolveApmClawPackageRoot } = await import("../infra/openclaw-root.js");
...
describe("update-cli", () => {
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveApmClawPackageRoot).mockResolvedValue(root);
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(resolveApmClawPackageRoot).mockResolvedValue(process.cwd());
  ...
  it("updateStatusCommand prints table output", async () => {
    ...
    expect(logs.join("\n")).toContain("ApmClaw update status");
  });
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as ApmClawConfig,
        });
      },
  ...
  it("honors --tag override", async () => {
    ...
    vi.mocked(resolveApmClawPackageRoot).mockResolvedValue(tempDir);
    vi.mocked(runGatewayUpdate).mockResolvedValue(
      makeOkUpdateResult({
        mode: "npm",
      }),
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ APMCLAW_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        vi.mocked(doctorCommand).mockResolvedValue(undefined);
        vi.mocked(defaultRuntime.log).mockClear();
        ...
        expect(process.env.APMCLAW_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    ...
    await withEnvAsync({ APMCLAW_GIT_DIR: tempDir }, async () => {
      setTty(true);
