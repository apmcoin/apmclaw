...
async function withOutsideHardlinkInApmClawTmp<T>(
  params: {
    apmClawTmpDir: string;
  ...
  const hardlinkPath = path.join(params.apmClawTmpDir, makeTmpProbePath(params.hardlinkPrefix));
  const symlinkPath = params.symlinkPrefix
    ? path.join(params.apmClawTmpDir, makeTmpProbePath(params.symlinkPrefix))
    : undefined;
  try {
    if (isPathInside(params.apmClawTmpDir, outsideFile)) {
      return;
    }
    await fs.writeFile(outsideFile, "secret", "utf8");
    await fs.mkdir(params.apmClawTmpDir, { recursive: true });
    try {
...
describe("resolveSandboxedMediaSource", () => {
  const apmClawTmpDir = resolvePreferredApmClawTmpDir();

  // Group 1: /tmp paths (the bug fix)
  it.each([
    {
      name: "absolute paths under preferred ApmClaw tmp root",
      media: path.join(apmClawTmpDir, "image.png"),
      expected: path.join(apmClawTmpDir, "image.png"),
    },
    {
      name: "file:// URLs pointing to preferred ApmClaw tmp root",
      media: pathToFileURL(path.join(apmClawTmpDir, "photo.png")).href,
      expected: path.join(apmClawTmpDir, "photo.png"),
    },
    {
      name: "nested paths under preferred ApmClaw tmp root",
      media: path.join(apmClawTmpDir, "subdir", "deep", "file.png"),
      expected: path.join(apmClawTmpDir, "subdir", "deep", "file.png"),
    },
  ...
  it.each([
    ...
    },
    {
      ...
      media: path.join(apmClawTmpDir, "..", "etc", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked ApmClaw tmp paths escaping tmp root", async () => {
    if (process.platform === "win32") {
      return;
    }
    const outsideTmpTarget = path.resolve(process.cwd(), "package.json");
    if (isPathInside(apmClawTmpDir, outsideTmpTarget)) {
      return;
    }

    await withSandboxRoot(async (sandboxDir) => {
      await fs.access(outsideTmpTarget);
      await fs.mkdir(apmClawTmpDir, { recursive: true });
      const symlinkPath = path.join(apmClawTmpDir, `tmp-link-escape-${process.pid}`);
  ...
  it("rejects hardlinked ApmClaw tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInApmClawTmp(
      {
        apmClawTmpDir,
  ...
  it("rejects symlinked ApmClaw tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInApmClawTmp(
