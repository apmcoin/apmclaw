...
beforeEach(() => {
  ...
  originalHideBanner = process.env.APMCLAW_HIDE_BANNER;
  delete process.env.NODE_NO_WARNINGS;
  delete process.env.APMCLAW_HIDE_BANNER;
});
...
afterEach(() => {
  ...
  if (originalHideBanner === undefined) {
    ...
    process.env.APMCLAW_HIDE_BANNER = originalHideBanner;
  }
});
...
describe("registerPreActionHooks", () => {
  ...
  it("skips help/version preaction and respects banner opt-out", async () => {
    ...
    vi.clearAllMocks();
    process.env.APMCLAW_HIDE_BANNER = "1";
