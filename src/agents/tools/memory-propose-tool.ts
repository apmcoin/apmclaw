...
const MemoryProposeSchema = Type.Object({
  ...
  reasoning: Type.String({
    description: "Detailed explanation of why this is spam (reference AGENTS.md patterns, mention red flags)"
  })
});
...
export function createMemoryProposeTool(options: {
  ...
  return {
    ...
    name: "memory_propose",
    description: "Propose a new spam pattern for admin approval. Use when you detect suspicious patterns not yet in MEMORY.md. Admins review via Telegram (button=approve, reply=reject). Do NOT use for patterns already in AGENTS.md or MEMORY.md Approved Patterns - just delete those immediately.",
    ...
    execute: async (_toolCallId, params) => {
      ...
      const actionTaken = readStringParam(params, "actionTaken", { required: true }) as "deleted" | "preserved";
