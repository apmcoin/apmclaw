...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredApmClawTmpDir: vi.fn(() => "/tmp/openclaw"),
}));
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredApmClawTmpDir.mockReturnValue("/tmp/openclaw");
  });
  ...
  async function expectAtomicDownloadSave(params: {
    ...
    expect(path.basename(String(savedPath))).toContain(".apmclaw-output-");
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredApmClawTmpDir.mockReturnValue("/tmp/openclaw-preferred");
    ...
    expect(tmpDirMocks.resolvePreferredApmClawTmpDir).toHaveBeenCalled();
  });
