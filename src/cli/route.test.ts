...
describe("tryRouteCli", () => {
  ...
  beforeEach(async () => {
    vi.clearAllMocks();
    originalDisableRouteFirst = process.env.APMCLAW_DISABLE_ROUTE_FIRST;
    delete process.env.APMCLAW_DISABLE_ROUTE_FIRST;
    vi.resetModules();
  ...
  afterEach(() => {
    if (originalDisableRouteFirst === undefined) {
      ...
      process.env.APMCLAW_DISABLE_ROUTE_FIRST = originalDisableRouteFirst;
    }
  });
