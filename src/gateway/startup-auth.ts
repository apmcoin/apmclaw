...
import type { GatewayAuthConfig, GatewayTailscaleConfig, ApmClawConfig } from "../config/config.js";
...
function hasGatewayTokenCandidate(params: {
  ...
  const envToken =
    params.env.APMCLAW_GATEWAY_TOKEN?.trim() || params.env.CLAWDBOT_GATEWAY_TOKEN?.trim();
  if (envToken) {
    return true;
  }
  if (
...
function hasGatewayTokenEnvCandidate(env: NodeJS.ProcessEnv): boolean {
  return Boolean(env.APMCLAW_GATEWAY_TOKEN?.trim() || env.CLAWDBOT_GATEWAY_TOKEN?.trim());
}

function hasGatewayPasswordEnvCandidate(env: NodeJS.ProcessEnv): boolean {
  return Boolean(env.APMCLAW_GATEWAY_PASSWORD?.trim() || env.CLAWDBOT_GATEWAY_PASSWORD?.trim());
}
