...
import { createApmClawCodingTools } from "./pi-tools.js";
...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    try {
      ...
      const tools = createApmClawCodingTools({
        config: cfg,
...
*** End Patch`;
  ...
  function createMainSessionTools(cfg: ApmClawConfig) {
  ...
  function createMainAgentConfig(params: {
    tools: NonNullable<ApmClawConfig["tools"]>;
    agentTools?: NonNullable<NonNullable<ApmClawConfig["agents"]>["list"]>[number]["tools"];
  }): ApmClawConfig {
    return {
      tools: params.tools,
      agents: {
  ...
  it("should allow different tool policies for different agents", () => {
    ...
    const mainTools = createApmClawCodingTools({
      config: cfg,
      sessionKey: "agent:main:main",
    ...
    const familyTools = createApmClawCodingTools({
      config: cfg,
  ...
  it("should apply group tool policy overrides (group-specific beats wildcard)", () => {
    ...
    const trustedTools = createApmClawCodingTools({
      config: cfg,
    ...
    const defaultTools = createApmClawCodingTools({
      config: cfg,
  ...
  it("should apply per-sender tool policies for group tools", () => {
    ...
    const aliceTools = createApmClawCodingTools({
      config: cfg,
    ...
    const bobTools = createApmClawCodingTools({
      config: cfg,
      sessionKey: "agent:main:whatsapp:group:family",
      senderId: "bob",
  ...
  it("should not let default sender policy override group tools", () => {
    ...
    const adminTools = createApmClawCodingTools({
      config: cfg,
  ...
  it("should apply agent-specific exec host defaults over global defaults", async () => {
    ...
    const helperTools = createApmClawCodingTools({
      config: cfg,
