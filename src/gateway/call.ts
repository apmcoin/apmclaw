...
export function buildGatewayConnectionDetails(
  ...
  const envUrlOverride = cliUrlOverride
    ? undefined
    : (trimToUndefined(process.env.APMCLAW_GATEWAY_URL) ??
  ...
  const allowPrivateWs = process.env.APMCLAW_ALLOW_INSECURE_PRIVATE_WS === "1";
  ...
  if (!isSecureWebSocketUrl(url, { allowPrivateWs })) {
    throw new Error(
      [
        ...
        allowPrivateWs
          ? undefined
          : "Break-glass (trusted private networks only): set APMCLAW_ALLOW_INSECURE_PRIVATE_WS=1",
        "Doctor: openclaw doctor --fix",
        "Docs: https://docs.apmclaw.ai/gateway/remote",
      ].join("\n"),
    );
  }
...
function readGatewayTokenEnv(env: NodeJS.ProcessEnv): string | undefined {
  return trimToUndefined(env.APMCLAW_GATEWAY_TOKEN) ?? trimToUndefined(env.CLAWDBOT_GATEWAY_TOKEN);
}

function readGatewayPasswordEnv(env: NodeJS.ProcessEnv): string | undefined {
  return (
    trimToUndefined(env.APMCLAW_GATEWAY_PASSWORD) ?? trimToUndefined(env.CLAWDBOT_GATEWAY_PASSWORD)
  );
}
