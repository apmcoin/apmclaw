...
const MISSING_EXTENSIONS_ERROR =
  'package.json missing openclaw.extensions; update the plugin package to include openclaw.extensions (for example ["./dist/index.js"]). See https://docs.apmclaw.ai/help/troubleshooting#plugin-install-fails-with-missing-openclaw-extensions';

export const PLUGIN_INSTALL_ERROR_CODE = {
  INVALID_NPM_SPEC: "invalid_npm_spec",
  MISSING_APMCLAW_EXTENSIONS: "missing_openclaw_extensions",
  EMPTY_APMCLAW_EXTENSIONS: "empty_openclaw_extensions",
...
function ensureApmClawExtensions(params: { manifest: PackageManifest }):
  | {
      ok: true;
      entries: string[];
    }
  ...
  if (resolved.status === "missing") {
    return {
      ...
      code: PLUGIN_INSTALL_ERROR_CODE.MISSING_APMCLAW_EXTENSIONS,
    };
  }
  if (resolved.status === "empty") {
    return {
      ...
      code: PLUGIN_INSTALL_ERROR_CODE.EMPTY_APMCLAW_EXTENSIONS,
    };
  }
  return {
    ok: true,
...
async function installPluginFromPackageDir(
  ...
  const extensionsResult = ensureApmClawExtensions({
    manifest,
  });
