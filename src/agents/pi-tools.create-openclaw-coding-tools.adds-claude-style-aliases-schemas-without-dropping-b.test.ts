...
const defaultTools = createApmClawCodingTools({ senderIsOwner: true });
...
describe("createApmClawCodingTools", () => {
  ...
  it("enforces apply_patch availability and canonical names across model/provider constraints", () => {
    ...
    const enabledConfig: ApmClawConfig = {
      tools: {
        exec: {
    ...
    const openAiTools = createApmClawCodingTools({
    ...
    const anthropicTools = createApmClawCodingTools({
      config: enabledConfig,
    ...
    const allowModelsConfig: ApmClawConfig = {
      tools: {
        exec: {
    ...
    const allowed = createApmClawCodingTools({
    ...
    const denied = createApmClawCodingTools({
      config: allowModelsConfig,
      modelProvider: "openai",
      modelId: "gpt-5-mini",
    });
    ...
    const oauthTools = createApmClawCodingTools({
      modelProvider: "anthropic",
      modelAuthMode: "oauth",
    });
