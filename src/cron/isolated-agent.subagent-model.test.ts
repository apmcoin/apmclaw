...
async function writeSessionStore(home: string) {
  const dir = path.join(home, ".apmclaw", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<ApmClawConfig> = {},
): ApmClawConfig {
  const base: ApmClawConfig = {
    agents: {
      defaults: {
...
async function runSubagentModelCase(params: {
  home: string;
  cfgOverrides?: Partial<ApmClawConfig>;
  ...
  it.each([
    {
      ...
      } satisfies Partial<ApmClawConfig>,
