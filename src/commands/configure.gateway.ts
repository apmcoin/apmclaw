...
export async function promptGatewayConfig(
    ...
    if (tokenInputMode === "ref") {
      const envVar = guardCancel(
        await text({
          ...
          initialValue: "APMCLAW_GATEWAY_TOKEN",
          placeholder: "APMCLAW_GATEWAY_TOKEN",
          validate: (value) => {
            const candidate = String(value ?? "").trim();
            if (!isValidEnvSecretRefId(candidate)) {
              return "Use an env var name like APMCLAW_GATEWAY_TOKEN.";
            }
      ...
      note(`Validated ${envVarName}. ApmClaw will store a token SecretRef.`, "Gateway token");
    } else {
  ...
  if (authMode === "trusted-proxy") {
    note(
      [
        "Trusted proxy mode: ApmClaw trusts user identity from a reverse proxy.",
        ...
        "Docs: https://docs.apmclaw.ai/gateway/trusted-proxy-auth",
      ].join("\n"),
      "Trusted Proxy Auth",
    );
