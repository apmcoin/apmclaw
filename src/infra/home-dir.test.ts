...
describe("resolveEffectiveHomeDir", () => {
  it("prefers APMCLAW_HOME over HOME and USERPROFILE", () => {
    const env = {
  ...
  it("expands APMCLAW_HOME when set to ~", () => {
    const env = {
      APMCLAW_HOME: "~/svc",
      HOME: "/home/alice",
    } as NodeJS.ProcessEnv;
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for APMCLAW_HOME", () => {
    const result = resolveRequiredHomeDir(
      { APMCLAW_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when APMCLAW_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ APMCLAW_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
...
describe("expandHomePrefix", () => {
  it("expands tilde using effective home", () => {
    const value = expandHomePrefix("~/x", {
      env: { APMCLAW_HOME: "/srv/openclaw-home" } as NodeJS.ProcessEnv,
    });
