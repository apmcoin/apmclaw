...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: ApmClawConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: ApmClawConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: ApmClawConfig, agentId?: string): boolean {
...
function resolveHeartbeatConfig(cfg: ApmClawConfig, agentId?: string): HeartbeatConfig | undefined {
...
function resolveHeartbeatAgents(cfg: ApmClawConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: ApmClawConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: ApmClawConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: ApmClawConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
