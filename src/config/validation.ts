...
import type { ApmClawConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: ApmClawConfig): ConfigValidationIssue[] {
...
function validateGatewayTailscaleBind(config: ApmClawConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: ApmClawConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = ApmClawSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as ApmClawConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as ApmClawConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  const gatewayTailscaleBindIssues = validateGatewayTailscaleBind(validated.data as ApmClawConfig);
  if (gatewayTailscaleBindIssues.length > 0) {
    return { ok: false, issues: gatewayTailscaleBindIssues };
  }
  return {
    ok: true,
    config: validated.data as ApmClawConfig,
  };
}
