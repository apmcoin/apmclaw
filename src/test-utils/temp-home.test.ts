...
describe("createTempHomeEnv", () => {
  it("sets home env vars and restores them on cleanup", async () => {
    ...
    const previousStateDir = process.env.APMCLAW_STATE_DIR;
    ...
    expect(process.env.APMCLAW_STATE_DIR).toBe(path.join(tempHome.home, ".apmclaw"));
    ...
    expect(process.env.APMCLAW_STATE_DIR).toBe(previousStateDir);
