...
describe("update-startup", () => {
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["APMCLAW_STATE_DIR", "NODE_ENV", "VITEST"]);
    process.env.APMCLAW_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveApmClawPackageRoot } = await import("./openclaw-root.js"));
    ...
    vi.mocked(resolveApmClawPackageRoot).mockClear();
  ...
  function mockPackageInstallStatus() {
    vi.mocked(resolveApmClawPackageRoot).mockResolvedValue("/opt/openclaw");
