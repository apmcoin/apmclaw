...
describe("sessions", () => {
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ APMCLAW_STATE_DIR: stateDir }, fn);
  ...
  it("derives session transcripts dir from APMCLAW_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { APMCLAW_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when APMCLAW_STATE_DIR differs from stored paths", () => {
