...
type AgentEntry = NonNullable<NonNullable<ApmClawConfig["agents"]>["list"]>[number];
...
export function listAgentEntries(cfg: ApmClawConfig): AgentEntry[] {
...
export function listAgentIds(cfg: ApmClawConfig): string[] {
...
export function resolveDefaultAgentId(cfg: ApmClawConfig): string {
  const agents = listAgentEntries(cfg);
  if (agents.length === 0) {
    return DEFAULT_AGENT_ID;
  }
...
function resolveAgentEntry(cfg: ApmClawConfig, agentId: string): AgentEntry | undefined {
...
export function resolveAgentModelPrimary(cfg: ApmClawConfig, agentId: string): string | undefined {
...
export function resolveAgentWorkspaceDir(cfg: ApmClawConfig, agentId: string) {
  const id = normalizeAgentId(agentId);
...
export function resolveAgentDir(cfg: ApmClawConfig, agentId: string) {
  const id = normalizeAgentId(agentId);
