...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: ApmClawConfig | null = null;
  ...
  get(key: string, cfg: ApmClawConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: ApmClawConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: ApmClawConfig): void {
