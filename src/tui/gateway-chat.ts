...
function throwGatewayAuthResolutionError(reason: string): never {
  throw new Error(
    [
      reason,
      "Fix: set APMCLAW_GATEWAY_TOKEN/APMCLAW_GATEWAY_PASSWORD, pass --token/--password,",
...
export async function resolveGatewayConnection(
  ...
  const envToken = trimToUndefined(env.APMCLAW_GATEWAY_TOKEN);
  const envPassword = trimToUndefined(env.APMCLAW_GATEWAY_PASSWORD);
