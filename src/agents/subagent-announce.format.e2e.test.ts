...
describe("subagent announce formatting", () => {
  ...
  beforeAll(async () => {
    ...
    previousFastTestEnv = process.env.APMCLAW_TEST_FAST;
    process.env.APMCLAW_TEST_FAST = "1";
    ({ runSubagentAnnounceFlow } = await import("./subagent-announce.js"));
  });

  afterAll(() => {
    if (previousFastTestEnv === undefined) {
      delete process.env.APMCLAW_TEST_FAST;
      return;
    }
    process.env.APMCLAW_TEST_FAST = previousFastTestEnv;
  });

  beforeEach(() => {
    // APMCLAW_TEST_FAST is set in beforeAll before module import
  ...
  it("sends instructional message to main agent with status and findings", async () => {
    ...
    expect(msg).toContain("ApmClaw runtime context (internal):");
