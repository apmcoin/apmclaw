...
describe("resolveSessionTranscriptCandidates", () => {
  ...
  test("fallback candidate uses APMCLAW_HOME instead of os.homedir()", () => {
    ...
    expect(fallback).toBe(
      path.join(path.resolve("/srv/openclaw-home"), ".apmclaw", "sessions", "sess-1.jsonl"),
    );
  });
});
...
describe("archiveSessionTranscripts", () => {
  ...
  beforeAll(() => {
    vi.stubEnv("APMCLAW_HOME", tmpDir);
  });
