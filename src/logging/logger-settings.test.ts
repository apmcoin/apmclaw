...
let originalApmClawLogLevel: string | undefined;
...
beforeEach(() => {
  originalTestFileLog = process.env.APMCLAW_TEST_FILE_LOG;
  originalApmClawLogLevel = process.env.APMCLAW_LOG_LEVEL;
  delete process.env.APMCLAW_TEST_FILE_LOG;
  delete process.env.APMCLAW_LOG_LEVEL;
...
afterEach(() => {
  if (originalTestFileLog === undefined) {
    ...
    process.env.APMCLAW_TEST_FILE_LOG = originalTestFileLog;
  }
  if (originalApmClawLogLevel === undefined) {
    delete process.env.APMCLAW_LOG_LEVEL;
  } else {
    process.env.APMCLAW_LOG_LEVEL = originalApmClawLogLevel;
  }
  logging.resetLogger();
  logging.setLoggerOverride(null);
  vi.restoreAllMocks();
...
describe("getResolvedLoggerSettings", () => {
  ...
  it("reads logging config when test file logging is explicitly enabled", () => {
    process.env.APMCLAW_TEST_FILE_LOG = "1";
    const settings = logging.getResolvedLoggerSettings();
