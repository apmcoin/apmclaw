...
function resolveAgentName(cfg: ApmClawConfig, agentId: string) {
...
function resolveAgentModel(cfg: ApmClawConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: ApmClawConfig): AgentSummary[] {
