...
import type { ApmClawConfig, ConfigFileSnapshot } from "../../config/types.js";
...
export function resolveTargets(cfg: ApmClawConfig, explicitUrl?: string): GatewayStatusTarget[] {
...
function readGatewayTokenEnv(env: NodeJS.ProcessEnv = process.env): string | undefined {
  const token = env.APMCLAW_GATEWAY_TOKEN?.trim() || env.CLAWDBOT_GATEWAY_TOKEN?.trim();
  return token || undefined;
}

function readGatewayPasswordEnv(env: NodeJS.ProcessEnv = process.env): string | undefined {
  const password = env.APMCLAW_GATEWAY_PASSWORD?.trim() || env.CLAWDBOT_GATEWAY_PASSWORD?.trim();
...
export function buildNetworkHints(cfg: ApmClawConfig) {
