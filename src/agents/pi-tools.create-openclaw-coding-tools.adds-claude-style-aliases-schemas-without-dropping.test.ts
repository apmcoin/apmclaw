...
import { __testing, createApmClawCodingTools } from "./pi-tools.js";
import { createApmClawReadTool, createSandboxedReadTool } from "./pi-tools.read.js";
...
describe("createApmClawCodingTools", () => {
  ...
  it("exposes raw for gateway config.apply tool calls", () => {
    const gateway = createApmClawCodingTools({ senderIsOwner: true }).find(
  ...
  it("keeps raw core tool schemas union-free", () => {
    const tools = createApmClawTools();
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createApmClawCodingTools({ messageProvider: "discord" });
  ...
  it("removes unsupported JSON Schema keywords for Cloud Code Assist API compatibility", () => {
    const googleTools = createApmClawCodingTools({
  ...
  it("strips truncation.content details from read results while preserving other fields", async () => {
    ...
    const wrapped = createApmClawReadTool(
      baseRead as unknown as Parameters<typeof createApmClawReadTool>[0],
    );
