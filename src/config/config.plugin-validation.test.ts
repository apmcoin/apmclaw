...
describe("config plugin validation", () => {
  ...
  const envSnapshot = {
    ...
    APMCLAW_PLUGIN_MANIFEST_CACHE_MS: process.env.APMCLAW_PLUGIN_MANIFEST_CACHE_MS,
  };
  ...
  beforeAll(async () => {
    ...
    process.env.APMCLAW_STATE_DIR = path.join(suiteHome, ".apmclaw");
    process.env.APMCLAW_PLUGIN_MANIFEST_CACHE_MS = "10000";
  ...
  afterAll(async () => {
    ...
    clearPluginManifestRegistryCache();
    if (envSnapshot.APMCLAW_STATE_DIR === undefined) {
      delete process.env.APMCLAW_STATE_DIR;
    } else {
      process.env.APMCLAW_STATE_DIR = envSnapshot.APMCLAW_STATE_DIR;
    }
    if (envSnapshot.APMCLAW_PLUGIN_MANIFEST_CACHE_MS === undefined) {
      delete process.env.APMCLAW_PLUGIN_MANIFEST_CACHE_MS;
    } else {
      process.env.APMCLAW_PLUGIN_MANIFEST_CACHE_MS = envSnapshot.APMCLAW_PLUGIN_MANIFEST_CACHE_MS;
    }
  });
