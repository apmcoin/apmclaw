...
export const POSIX_APMCLAW_TMP_DIR = "/tmp/openclaw";
const TMP_DIR_ACCESS_MODE = fs.constants.W_OK | fs.constants.X_OK;

type ResolvePreferredApmClawTmpDirOptions = {
...
export function resolvePreferredApmClawTmpDir(
  options: ResolvePreferredApmClawTmpDirOptions = {},
): string {
  ...
  const ensureTrustedFallbackDir = (): string => {
    ...
    if (state === "invalid") {
      ...
      throw new Error(`Unsafe fallback ApmClaw temp dir: ${fallbackPath}`);
    }
    try {
      mkdirSync(fallbackPath, { recursive: true, mode: 0o700 });
      chmodSync(fallbackPath, 0o700);
    } catch {
      throw new Error(`Unable to create fallback ApmClaw temp dir: ${fallbackPath}`);
    }
  ...
  const existingPreferredState = resolveDirState(POSIX_APMCLAW_TMP_DIR);
  if (existingPreferredState === "available") {
    return POSIX_APMCLAW_TMP_DIR;
  }
  if (existingPreferredState === "invalid") {
    if (tryRepairWritableBits(POSIX_APMCLAW_TMP_DIR)) {
      return POSIX_APMCLAW_TMP_DIR;
    }
  ...
  try {
    ...
    mkdirSync(POSIX_APMCLAW_TMP_DIR, { recursive: true, mode: 0o700 });
    chmodSync(POSIX_APMCLAW_TMP_DIR, 0o700);
    if (
      resolveDirState(POSIX_APMCLAW_TMP_DIR) !== "available" &&
      !tryRepairWritableBits(POSIX_APMCLAW_TMP_DIR)
    ) {
      return ensureTrustedFallbackDir();
    }
    return POSIX_APMCLAW_TMP_DIR;
