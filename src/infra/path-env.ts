...
function candidateBinDirs(opts: EnsureApmClawPathOpts): { prepend: string[]; append: string[] } {
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.APMCLAW_ALLOW_PROJECT_LOCAL_BIN);
 ...
 */
export function ensureApmClawCliOnPath(opts: EnsureApmClawPathOpts = {}) {
  if (isTruthyEnvValue(process.env.APMCLAW_PATH_BOOTSTRAPPED)) {
    return;
  }
