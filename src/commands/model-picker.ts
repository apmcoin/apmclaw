...
type PromptDefaultModelResult = { model?: string; config?: ApmClawConfig };
...
function resolveConfiguredModelRaw(cfg: ApmClawConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: ApmClawConfig): string[] {
...
export function applyPrimaryModel(cfg: ApmClawConfig, model: string): ApmClawConfig {
...
export function applyModelAllowlist(cfg: ApmClawConfig, models: string[]): ApmClawConfig {
  const defaults = cfg.agents?.defaults;
