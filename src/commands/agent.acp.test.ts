...
function createAcpEnabledConfig(home: string, storePath: string): ApmClawConfig {
  return {
    acp: {
      enabled: true,
      backend: "acpx",
...
function mockConfigWithAcpOverrides(
  home: string,
  storePath: string,
  acpOverrides: Partial<NonNullable<ApmClawConfig["acp"]>>,
) {
  ...
  it.each([
    {
      ...
      acpOverrides: { enabled: false } satisfies Partial<NonNullable<ApmClawConfig["acp"]>>,
    },
    {
      name: "blocks ACP turns when ACP dispatch is disabled by policy",
      acpOverrides: {
        dispatch: { enabled: false },
      } satisfies Partial<NonNullable<ApmClawConfig["acp"]>>,
    },
