...
const bundledPluginsDirSnapshot = captureEnv(["APMCLAW_BUNDLED_PLUGINS_DIR"]);

beforeAll(() => {
  process.env.APMCLAW_BUNDLED_PLUGINS_DIR = path.join(
    os.tmpdir(),
...
const { createApmClawCodingTools } = await import("./pi-tools.js");
...
describe("createApmClawCodingTools safeBins", () => {
