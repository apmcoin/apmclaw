...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.APMCLAW_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".apmclaw", `workspace-${profile}`);
  }
  return path.join(home, ".apmclaw", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".apmclaw";
