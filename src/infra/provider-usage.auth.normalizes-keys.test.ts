...
describe("resolveProviderAuths key normalization", () => {
  ...
  async function withSuiteHome<T>(
    ...
    await fs.mkdir(path.join(base, ".apmclaw", "agents", "main", "sessions"), { recursive: true });
    ...
    process.env.APMCLAW_STATE_DIR = path.join(base, ".apmclaw");
  ...
  async function writeAuthProfiles(home: string, profiles: Record<string, unknown>) {
    const agentDir = path.join(home, ".apmclaw", "agents", "main", "agent");
  ...
  async function writeConfig(home: string, config: Record<string, unknown>) {
    const stateDir = path.join(home, ".apmclaw");
    await fs.mkdir(stateDir, { recursive: true });
    await fs.writeFile(
      path.join(stateDir, "apmclaw.json"),
