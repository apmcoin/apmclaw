...
describe("resolveAcpClientSpawnEnv", () => {
  it("sets APMCLAW_SHELL marker and preserves existing env values", () => {
    const env = resolveAcpClientSpawnEnv({
      PATH: "/usr/bin",
      USER: "openclaw",
    });

    expect(env.APMCLAW_SHELL).toBe("acp-client");
    expect(env.PATH).toBe("/usr/bin");
    expect(env.USER).toBe("openclaw");
  });

  it("overrides pre-existing APMCLAW_SHELL to acp-client", () => {
    const env = resolveAcpClientSpawnEnv({
      APMCLAW_SHELL: "wrong",
    });
    expect(env.APMCLAW_SHELL).toBe("acp-client");
