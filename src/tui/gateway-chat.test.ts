...
describe("resolveGatewayConnection", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["APMCLAW_GATEWAY_TOKEN", "APMCLAW_GATEWAY_PASSWORD"]);
  ...
  it.each([
    ...
    const result = await withEnvAsync({ APMCLAW_GATEWAY_TOKEN: "env-token" }, async () => {
  ...
  it("uses APMCLAW_GATEWAY_TOKEN for local mode", async () => {
    loadConfig.mockReturnValue({ gateway: { mode: "local" } });
  ...
  it("prefers APMCLAW_GATEWAY_PASSWORD over remote password fallback", async () => {
    loadConfig.mockReturnValue({
      gateway: {
        mode: "remote",
    ...
    await withEnvAsync({ APMCLAW_GATEWAY_PASSWORD: "env-pass" }, async () => {
      const result = await resolveGatewayConnection({});
