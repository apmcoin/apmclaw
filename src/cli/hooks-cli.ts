...
function buildHooksReport(config: ApmClawConfig): HookStatusReport {
...
function enableInternalHookEntries(config: ApmClawConfig, hookNames: string[]): ApmClawConfig {
...
export function registerHooksCli(program: Command): void {
  const hooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/hooks", "docs.apmclaw.ai/cli/hooks")}\n`,
    );
