...
describe("detectMacCloudSyncedStateDir", () => {
  ...
  it("detects state dir under Library/CloudStorage", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "Dropbox", "ApmClaw", ".apmclaw");
  ...
  it("ignores cloud-synced symlink prefix when resolved target is local", () => {
    ...
    const resolvedLocalPath = path.join(home, ".apmclaw");
  ...
  it("anchors cloud detection to OS homedir when APMCLAW_HOME is overridden", () => {
    const stateDir = path.join(home, "Library", "CloudStorage", "iCloud Drive", ".apmclaw");
    const originalApmClawHome = process.env.APMCLAW_HOME;
    process.env.APMCLAW_HOME = "/tmp/openclaw-home-override";
    ...
    try {
      ...
      if (originalApmClawHome === undefined) {
        delete process.env.APMCLAW_HOME;
      } else {
        process.env.APMCLAW_HOME = originalApmClawHome;
      }
    }
  });
