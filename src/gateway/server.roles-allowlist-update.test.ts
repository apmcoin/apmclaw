...
const connectNodeClient = async (params: {
  ...
  const token = process.env.APMCLAW_GATEWAY_TOKEN;
  if (!token) {
    throw new Error("APMCLAW_GATEWAY_TOKEN is required for node test clients");
  }
...
describe("gateway update.run", () => {
  test("writes sentinel and schedules restart", async () => {
    ...
    try {
      ...
      const sentinelPath = path.join(os.homedir(), ".apmclaw", "restart-sentinel.json");
