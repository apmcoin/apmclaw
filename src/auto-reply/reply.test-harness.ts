...
type HomeEnvSnapshot = {
  ...
  APMCLAW_AGENT_DIR: string | undefined;
...
function snapshotHomeEnv(): HomeEnvSnapshot {
  return {
    ...
    APMCLAW_AGENT_DIR: process.env.APMCLAW_AGENT_DIR,
...
export function createTempHomeHarness(options: { prefix: string; beforeEachCase?: () => void }) {
  ...
  async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    process.env.APMCLAW_AGENT_DIR = path.join(home, ".apmclaw", "agent");
    process.env.PI_CODING_AGENT_DIR = path.join(home, ".apmclaw", "agent");
