...
const ETC_APMCLAW_DIR = path.join(ROOT_DIR, "etc", "openclaw");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "apmclaw.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcApmClawPath(...parts: string[]) {
  return path.join(ETC_APMCLAW_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("rejects absolute path outside config directory (CWE-22)", () => {
    const absolute = etcApmClawPath("agents.json");
  ...
  describe("edge cases", () => {
    ...
    it("allows include files when the config root path is a symlink", async () => {
      ...
      try {
        ...
        const result = resolveConfigIncludes(
          ...
          path.join(linkRoot, "apmclaw.json"),
        );
    ...
    it("rejects oversized include files", async () => {
      ...
      try {
        ...
        expect(() =>
          resolveConfigIncludes({ $include: "./big.json5" }, path.join(configDir, "apmclaw.json")),
