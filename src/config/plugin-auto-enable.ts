...
function isGenericChannelConfigured(cfg: ApmClawConfig, channelId: string): boolean {
  const entry = resolveChannelConfig(cfg, channelId);
  return recordHasKeys(entry);
}
...
function collectModelRefs(cfg: ApmClawConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: ApmClawConfig, providerId: string): boolean {
...
function collectCandidateChannelIds(cfg: ApmClawConfig): string[] {
...
function isPluginExplicitlyDisabled(cfg: ApmClawConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: ApmClawConfig, pluginId: string): boolean {
...
function registerPluginEntry(cfg: ApmClawConfig, pluginId: string): ApmClawConfig {
  const builtInChannelId = normalizeChatChannelId(pluginId);
  if (builtInChannelId) {
    const channels = cfg.channels as Record<string, unknown> | undefined;
