...
const shouldRegisterPrimaryOnly = (argv: string[]) => {
  if (isTruthyEnvValue(process.env.APMCLAW_DISABLE_LAZY_SUBCOMMANDS)) {
    return false;
  }
...
const shouldEagerRegisterSubcommands = (_argv: string[]) => {
  return isTruthyEnvValue(process.env.APMCLAW_DISABLE_LAZY_SUBCOMMANDS);
};

const loadConfig = async (): Promise<ApmClawConfig> => {
...
const entries: SubCliEntry[] = [
  ...
  {
    name: "docs",
    description: "Search the live ApmClaw docs",
    hasSubcommands: false,
    register: async (program) => {
