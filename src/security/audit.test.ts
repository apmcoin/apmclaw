...
function stubChannelPlugin(params: {
  ...
  label: string;
  resolveAccount: (cfg: ApmClawConfig, accountId: string | null | undefined) => unknown;
  inspectAccount?: (cfg: ApmClawConfig, accountId: string | null | undefined) => unknown;
  listAccountIds?: (cfg: ApmClawConfig) => string[];
  isConfigured?: (account: unknown, cfg: ApmClawConfig) => boolean;
  isEnabled?: (account: unknown, cfg: ApmClawConfig) => boolean;
}): ChannelPlugin {
  return {
    id: params.id,
    meta: {
...
describe("security audit", () => {
  ...
  const createFilesystemAuditFixture = async (label: string) => {
    ...
    const configPath = path.join(stateDir, "apmclaw.json");
  ...
  const withChannelSecurityStateDir = async (fn: (tmp: string) => Promise<void>) => {
    ...
    await withEnvAsync({ APMCLAW_STATE_DIR: sharedChannelSecurityStateDir }, () =>
...
`,
  ...
  it("flags non-loopback bind without auth as critical", async () => {
    ...
    const prevToken = process.env.APMCLAW_GATEWAY_TOKEN;
    const prevPassword = process.env.APMCLAW_GATEWAY_PASSWORD;
    ...
    try {
      ...
      if (prevToken === undefined) {
        ...
        process.env.APMCLAW_GATEWAY_TOKEN = prevToken;
      }
      if (prevPassword === undefined) {
        delete process.env.APMCLAW_GATEWAY_PASSWORD;
      } else {
        process.env.APMCLAW_GATEWAY_PASSWORD = prevPassword;
      }
    }
  });
  ...
  it("uses symlink target permissions for config checks", async () => {
    ...
    const targetConfigPath = path.join(tmp, "managed-apmclaw.json");
  ...
  it("flags wildcard Control UI origins by exposure level", async () => {
    const loopbackCfg: ApmClawConfig = {
      gateway: {
        bind: "loopback",
        controlUi: { allowedOrigins: ["*"] },
      },
    };
    const exposedCfg: ApmClawConfig = {
      gateway: {
        bind: "lan",
        auth: { mode: "token", token: "very-long-browser-token-0123456789" },
        controlUi: { allowedOrigins: ["*"] },
  ...
  it("scores X-Real-IP fallback risk by gateway exposure", async () => {
    const trustedProxyCfg = (trustedProxies: string[]): ApmClawConfig => ({
      gateway: {
        bind: "loopback",
  ...
  it("warns when multiple DM senders share the main session", async () => {
    const cfg: ApmClawConfig = { session: { dmScope: "main" } };
  ...
  it("keeps channel security findings when SecretRef credentials are configured but unavailable", async () => {
    await withChannelSecurityStateDir(async () => {
      const sourceConfig: ApmClawConfig = {
        channels: {
          discord: {
            enabled: true,
      ...
      const resolvedConfig: ApmClawConfig = {
        channels: {
          discord: {
            enabled: true,
            groupPolicy: "allowlist",
  ...
  it("warns when Discord allowlists contain name-based entries", async () => {
    await withChannelSecurityStateDir(async (tmp) => {
      ...
      expect(finding?.detail).toContain("~/.apmclaw/credentials/discord-allowFrom.json:team.owner");
  ...
  it("adds probe_failed warnings for deep probe failure modes", async () => {
    const cfg: ApmClawConfig = { gateway: { mode: "local" } };
    const cases: Array<{
      name: string;
  ...
  it("flags hooks token reuse of the gateway env token as critical", async () => {
    const prevToken = process.env.APMCLAW_GATEWAY_TOKEN;
    process.env.APMCLAW_GATEWAY_TOKEN = "shared-gateway-token-1234567890";
    const cfg: ApmClawConfig = {
  ...
  it("scores hooks request sessionKey override by gateway exposure", async () => {
    ...
    } satisfies NonNullable<ApmClawConfig["hooks"]>;
    const cases: Array<{
      name: string;
      cfg: ApmClawConfig;
      expectedSeverity: "warn" | "critical";
  ...
  it("warns when state/config look like a synced folder", async () => {
    ...
    const res = await audit(cfg, {
      stateDir: "/Users/test/Dropbox/.apmclaw",
      configPath: "/Users/test/Dropbox/.apmclaw/apmclaw.json",
    });
  ...
  it("flags group/world-readable config include files", async () => {
    ...
    const cfg: ApmClawConfig = { logging: { redactSensitive: "off" } };
    const user = "DESKTOP-TEST\\Tester";
  ...
  it("warns on unpinned npm install specs and missing integrity metadata", async () => {
    ...
    const res = await runSecurityAudit({
      ...
      configPath: path.join(sharedInstallMetadataStateDir, "apmclaw.json"),
      execDockerRawFn: execDockerRawUnavailable,
    });
  ...
  describe("maybeProbeGateway auth selection", () => {
    ...
    const makeProbeEnv = (env?: { token?: string; password?: string }) => {
      ...
      if (env?.token !== undefined) {
        probeEnv.APMCLAW_GATEWAY_TOKEN = env.token;
      }
      if (env?.password !== undefined) {
        probeEnv.APMCLAW_GATEWAY_PASSWORD = env.password;
      }
      return probeEnv;
    };
