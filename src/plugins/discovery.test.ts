...
import { clearPluginDiscoveryCache, discoverApmClawPlugins } from "./discovery.js";
...
async function withStateDir<T>(stateDir: string, fn: () => Promise<T>) {
  return await withEnvAsync(
    {
      APMCLAW_STATE_DIR: stateDir,
      CLAWDBOT_STATE_DIR: undefined,
      APMCLAW_BUNDLED_PLUGINS_DIR: "/nonexistent/bundled/plugins",
    },
    fn,
  );
}
...
async function discoverWithStateDir(
  stateDir: string,
  params: Parameters<typeof discoverApmClawPlugins>[0],
) {
  return await withStateDir(stateDir, async () => {
    return discoverApmClawPlugins(params);
  });
}
...
describe("discoverApmClawPlugins", () => {
  it("discovers global and workspace extensions", async () => {
    ...
    const workspaceExt = path.join(workspaceDir, ".apmclaw", "extensions");
    fs.mkdirSync(workspaceExt, { recursive: true });
    fs.writeFileSync(path.join(workspaceExt, "beta.ts"), "export default function () {}", "utf-8");

    const { candidates } = await withStateDir(stateDir, async () => {
      return discoverApmClawPlugins({ workspaceDir });
    });
  ...
  it("treats configured directory paths as plugin packages", async () => {
    ...
    const { candidates } = await withStateDir(stateDir, async () => {
      return discoverApmClawPlugins({ extraPaths: [packDir] });
    });
  ...
  it.runIf(process.platform !== "win32" && typeof process.getuid === "function")(
    ...
    async () => {
      ...
      const result = await withStateDir(stateDir, async () => {
        return discoverApmClawPlugins({ ownershipUid: actualUid + 1 });
      });
  ...
  it("reuses discovery results from cache until cleared", async () => {
    ...
    const first = await withEnvAsync(
      {
        APMCLAW_PLUGIN_DISCOVERY_CACHE_MS: "5000",
      },
      async () => withStateDir(stateDir, async () => discoverApmClawPlugins({})),
    );
