...
const GATEWAY_TEST_ENV_KEYS = [
  ...
  "APMCLAW_STATE_DIR",
  "APMCLAW_CONFIG_PATH",
  "APMCLAW_SKIP_BROWSER_CONTROL_SERVER",
  "APMCLAW_SKIP_GMAIL_WATCHER",
  "APMCLAW_SKIP_CANVAS_HOST",
  "APMCLAW_BUNDLED_PLUGINS_DIR",
  "APMCLAW_SKIP_CHANNELS",
  "APMCLAW_SKIP_PROVIDERS",
  "APMCLAW_SKIP_CRON",
  "APMCLAW_TEST_MINIMAL_GATEWAY",
] as const;
...
async function setupGatewayTestHome() {
  ...
  process.env.APMCLAW_STATE_DIR = path.join(tempHome, ".apmclaw");
  delete process.env.APMCLAW_CONFIG_PATH;
}

function applyGatewaySkipEnv() {
  process.env.APMCLAW_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.APMCLAW_SKIP_GMAIL_WATCHER = "1";
  process.env.APMCLAW_SKIP_CANVAS_HOST = "1";
  process.env.APMCLAW_SKIP_CHANNELS = "1";
  process.env.APMCLAW_SKIP_PROVIDERS = "1";
  process.env.APMCLAW_SKIP_CRON = "1";
  process.env.APMCLAW_TEST_MINIMAL_GATEWAY = "1";
  process.env.APMCLAW_BUNDLED_PLUGINS_DIR = tempHome
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    const suiteRoot = path.join(tempHome, ".apmclaw-test-suite");
    ...
    tempConfigRoot = path.join(tempHome, ".apmclaw-test");
    await fs.rm(tempConfigRoot, { recursive: true, force: true });
    await fs.mkdir(tempConfigRoot, { recursive: true });
  }
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["APMCLAW_GATEWAY_TOKEN"]);
  const prev = process.env.APMCLAW_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.APMCLAW_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const safe =
    `${params.clientId}-${params.clientMode}-${params.platform}-${params.deviceFamily ?? "none"}-${params.role}`
      ...
      .toLowerCase();
  const suiteRoot = process.env.APMCLAW_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
