...
import type { ConfigFileSnapshot, ApmClawConfig } from "../config/config.js";
...
function isFeishuDocToolEnabled(cfg: ApmClawConfig): boolean {
  ...
  if (stateDirPerms.ok) {
    ...
    if (stateDirPerms.worldWritable) {
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; other users can write into your ApmClaw state.`,
      ...
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; group users can write into your ApmClaw state.`,
        remediation: formatPermissionRemediation({
          targetPath: params.stateDir,
          perms: stateDirPerms,
          isDir: true,
...
function collectGatewayConfigFindings(
  ...
  const envTokenConfigured =
    hasNonEmptyString(env.APMCLAW_GATEWAY_TOKEN) || hasNonEmptyString(env.CLAWDBOT_GATEWAY_TOKEN);
  const envPasswordConfigured =
    hasNonEmptyString(env.APMCLAW_GATEWAY_PASSWORD) ||
...
function collectBrowserControlFindings(
  ...
  const tokenConfigured =
    ...
    hasNonEmptyString(env.APMCLAW_GATEWAY_TOKEN) ||
...
function collectLoggingFindings(cfg: ApmClawConfig): SecurityAuditFinding[] {
...
function collectElevatedFindings(cfg: ApmClawConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function collectExecRuntimeFindings(cfg: ApmClawConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export async function runSecurityAudit(opts: SecurityAuditOptions): Promise<SecurityAuditReport> {
  ...
  if (deepProbeResult?.authWarning) {
    findings.push({
      ...
      remediation: `Set APMCLAW_GATEWAY_TOKEN/APMCLAW_GATEWAY_PASSWORD in this shell or resolve the external secret provider, then re-run "${formatCliCommand("openclaw security audit --deep")}".`,
    });
  }
