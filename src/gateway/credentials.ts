  ...
  constructor(path: string) {
    super(
      [
        ...
        "Fix: set APMCLAW_GATEWAY_TOKEN/APMCLAW_GATEWAY_PASSWORD, pass explicit --token/--password,",
...
function readGatewayTokenEnv(
  ...
  const primary = trimToUndefined(env.APMCLAW_GATEWAY_TOKEN);
  if (primary) {
    return primary;
  }
  if (!includeLegacyEnv) {
...
function readGatewayPasswordEnv(
  ...
  const primary = trimToUndefined(env.APMCLAW_GATEWAY_PASSWORD);
  if (primary) {
    return primary;
  }
  if (!includeLegacyEnv) {
