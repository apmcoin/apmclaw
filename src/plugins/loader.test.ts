...
import { __testing, loadApmClawPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const fixtureRoot = fs.mkdtempSync(path.join(os.tmpdir(), "openclaw-plugin-"));
let tempDirIndex = 0;
const prevBundledDir = process.env.APMCLAW_BUNDLED_PLUGINS_DIR;
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  if (!options && cachedBundledMemoryDir) {
    process.env.APMCLAW_BUNDLED_PLUGINS_DIR = cachedBundledMemoryDir;
    return loadApmClawPlugins({
      cache: false,
  ...
  process.env.APMCLAW_BUNDLED_PLUGINS_DIR = bundledDir;
...
function setupBundledTelegramPlugin() {
  ...
  process.env.APMCLAW_BUNDLED_PLUGINS_DIR = cachedBundledTelegramDir;
}

function expectTelegramLoaded(registry: ReturnType<typeof loadApmClawPlugins>) {
...
function useNoBundledPlugins() {
  process.env.APMCLAW_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
}
...
function loadRegistryFromSinglePlugin(params: {
  ...
  options?: Omit<Parameters<typeof loadApmClawPlugins>[0], "cache" | "workspaceDir" | "config">;
}) {
...
afterEach(() => {
  if (prevBundledDir === undefined) {
    delete process.env.APMCLAW_BUNDLED_PLUGINS_DIR;
  } else {
    process.env.APMCLAW_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("loadApmClawPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    const registry = loadApmClawPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
  ...
  it("re-initializes global hook runner when serving registry from cache", () => {
    ...
    const first = loadApmClawPlugins(options);
    expect(getGlobalHookRunner()).not.toBeNull();

    resetGlobalHookRunner();
    expect(getGlobalHookRunner()).toBeNull();

    const second = loadApmClawPlugins(options);
  ...
  it("prefers bundled plugin over auto-discovered global duplicate ids", () => {
    ...
    withEnv({ APMCLAW_STATE_DIR: stateDir, CLAWDBOT_STATE_DIR: undefined }, () => {
