...
export const resolveApmClawPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  loadApmClawPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
beforeEach(() => {
  ...
  resolveApmClawPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.APMCLAW_STATE_DIR;
  originalUpdateInProgress = process.env.APMCLAW_UPDATE_IN_PROGRESS;
  process.env.APMCLAW_UPDATE_IN_PROGRESS = "1";
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.APMCLAW_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.APMCLAW_UPDATE_IN_PROGRESS;
  } else {
    process.env.APMCLAW_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
