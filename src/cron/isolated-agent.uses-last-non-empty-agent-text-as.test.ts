...
type HomeEnvSnapshot = {
  ...
  APMCLAW_HOME: string | undefined;
  APMCLAW_STATE_DIR: string | undefined;
};
...
function restoreHomeEnv(snapshot: HomeEnvSnapshot) {
  ...
  restoreValue("APMCLAW_HOME");
  restoreValue("APMCLAW_STATE_DIR");
}
...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  ...
  process.env.APMCLAW_STATE_DIR = path.join(home, ".apmclaw");
...
describe("runCronIsolatedAgentTurn", () => {
  ...
  it("passes resolved agentDir to runEmbeddedPiAgent", async () => {
    await withTempHome(async (home) => {
      ...
      expect(call?.agentDir).toBe(path.join(home, ".apmclaw", "agents", "main", "agent"));
    });
  });
  ...
  it("uses agentId for workspace, session key, and store paths", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = makeCfg(
        home,
        path.join(home, ".apmclaw", "agents", "{agentId}", "sessions", "sessions.json"),
        {
          agents: {
