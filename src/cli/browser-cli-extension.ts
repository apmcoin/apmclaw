...
export async function installChromeExtension(opts?: {
  ...
  if (!hasManifest(src)) {
    throw new Error("Bundled Chrome extension is missing. Reinstall ApmClaw and try again.");
  }
      ...
      defaultRuntime.error(
        info(
          [
            ...
            `- Pin “ApmClaw Browser Relay”, then click it on the tab (badge shows ON)`,
            "",
            `${theme.muted("Docs:")} ${formatDocsLink("/tools/chrome-extension", "docs.apmclaw.ai/tools/chrome-extension")}`,
          ].join("\n"),
        ),
      );
    });
        ...
        defaultRuntime.error(
          danger(
            [
              ...
              `Docs: ${formatDocsLink("/tools/chrome-extension", "docs.apmclaw.ai/tools/chrome-extension")}`,
            ].join("\n"),
          ),
        );
        defaultRuntime.exit(1);
