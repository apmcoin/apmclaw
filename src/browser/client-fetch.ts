...
function enhanceBrowserFetchError(url: string, err: unknown, timeoutMs: number): Error {
  ...
  const operatorHint = isLocal
    ? `Restart the ApmClaw gateway (ApmClaw.app menubar, or \`${formatCliCommand("openclaw gateway")}\`).`
  ...
  if (looksLikeTimeout) {
    return new Error(
      `Can't reach the ApmClaw browser control service (timed out after ${timeoutMs}ms). ${operatorHint} ${modelHint}`,
    );
  }
  return new Error(
    `Can't reach the ApmClaw browser control service. ${operatorHint} ${modelHint} (${msg})`,
  );
}
