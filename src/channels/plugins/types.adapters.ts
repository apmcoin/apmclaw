...
export type ChannelConfigAdapter<ResolvedAccount> = {
  ...
  resolveAccount: (cfg: ApmClawConfig, accountId?: string | null) => ResolvedAccount;
  inspectAccount?: (cfg: ApmClawConfig, accountId?: string | null) => unknown;
  defaultAccountId?: (cfg: ApmClawConfig) => string;
  setAccountEnabled?: (params: {
    cfg: ApmClawConfig;
    accountId: string;
    enabled: boolean;
  }) => ApmClawConfig;
  deleteAccount?: (params: { cfg: ApmClawConfig; accountId: string }) => ApmClawConfig;
  isEnabled?: (account: ResolvedAccount, cfg: ApmClawConfig) => boolean;
  disabledReason?: (account: ResolvedAccount, cfg: ApmClawConfig) => string;
  isConfigured?: (account: ResolvedAccount, cfg: ApmClawConfig) => boolean | Promise<boolean>;
  unconfiguredReason?: (account: ResolvedAccount, cfg: ApmClawConfig) => string;
  describeAccount?: (account: ResolvedAccount, cfg: ApmClawConfig) => ChannelAccountSnapshot;
...
export type ChannelGatewayContext<ResolvedAccount = unknown> = {
   ...
   * - Built-in channels (telegram, signal, etc.) typically don't use this field
   *   because they can directly import internal modules
   * - External plugins should check for undefined before using
   *
   * @since Plugin SDK 2026.2.19
   * @see {@link https://docs.apmclaw.ai/plugins/developing-plugins | Plugin SDK documentation}
   */
...
export type ChannelHeartbeatAdapter = {
  ...
  resolveRecipients?: (params: { cfg: ApmClawConfig; opts?: { to?: string; all?: boolean } }) => {
    recipients: string[];
    source: string;
  };
};
