...
describe("resolveApmClawAgentDir", () => {
      ...
      withEnv(
        {
          ...
          APMCLAW_AGENT_DIR: undefined,
          PI_CODING_AGENT_DIR: undefined,
        },
        () => {
          const resolved = resolveApmClawAgentDir();
  ...
  it("honors APMCLAW_AGENT_DIR overrides", async () => {
    await withTempStateDir((stateDir) => {
      const override = path.join(stateDir, "agent");
      withEnv(
        {
          APMCLAW_STATE_DIR: undefined,
          APMCLAW_AGENT_DIR: override,
          PI_CODING_AGENT_DIR: undefined,
        },
        () => {
          const resolved = resolveApmClawAgentDir();
  ...
  it("honors PI_CODING_AGENT_DIR when APMCLAW_AGENT_DIR is unset", async () => {
    await withTempStateDir((stateDir) => {
  ...
  it("prefers APMCLAW_AGENT_DIR over PI_CODING_AGENT_DIR when both are set", async () => {
    await withTempStateDir((stateDir) => {
      ...
      withEnv(
        {
          APMCLAW_STATE_DIR: undefined,
          APMCLAW_AGENT_DIR: primaryOverride,
