...
import { resolveApmClawPackageRoot, resolveApmClawPackageRootSync } from "./openclaw-root.js";
...
export async function resolveControlUiDistIndexPath(
  ...
  const packageRoot = await resolveApmClawPackageRoot({ argv1: normalized, moduleUrl });
  if (packageRoot) {
...
export function resolveControlUiRootSync(opts: ControlUiRootResolveOptions = {}): string | null {
  ...
  const packageRoot = resolveApmClawPackageRootSync({
    argv1,
    moduleUrl: opts.moduleUrl,
    cwd,
  });
