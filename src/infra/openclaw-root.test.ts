...
describe("resolveApmClawPackageRoot", () => {
  let resolveApmClawPackageRoot: typeof import("./openclaw-root.js").resolveApmClawPackageRoot;
  let resolveApmClawPackageRootSync: typeof import("./openclaw-root.js").resolveApmClawPackageRootSync;

  beforeAll(async () => {
    ({ resolveApmClawPackageRoot, resolveApmClawPackageRootSync } =
  ...
  it("resolves package root from .bin argv1", async () => {
    ...
    expect(resolveApmClawPackageRootSync({ argv1 })).toBe(pkgRoot);
  });
  ...
  it("resolves package root via symlinked argv1", async () => {
    ...
    expect(resolveApmClawPackageRootSync({ argv1: bin })).toBe(realPkg);
  });
  ...
  it("prefers moduleUrl candidates", async () => {
    ...
    expect(resolveApmClawPackageRootSync({ moduleUrl })).toBe(pkgRoot);
  });
  ...
  it("returns null for non-openclaw package roots", async () => {
    ...
    expect(resolveApmClawPackageRootSync({ cwd: pkgRoot })).toBeNull();
  });
  ...
  it("async resolver matches sync behavior", async () => {
    ...
    await expect(resolveApmClawPackageRoot({ cwd: pkgRoot })).resolves.toBe(pkgRoot);
  });

  it("async resolver returns null when no package roots exist", async () => {
    await expect(resolveApmClawPackageRoot({ cwd: fx("missing") })).resolves.toBeNull();
  });
});
