...
function telegramCfg(): ApmClawConfig {
  return { channels: { telegram: { botToken: "tok" } } } as ApmClawConfig;
}
...
async function runTelegramAction(
  ...
  options?: { cfg?: ApmClawConfig; accountId?: string },
) {
...
function slackHarness() {
  const cfg = { channels: { slack: { botToken: "tok" } } } as ApmClawConfig;
...
describe("discord message actions", () => {
  it("lists channel and upload actions by default", async () => {
    const cfg = { channels: { discord: { token: "d0" } } } as ApmClawConfig;
  ...
  it("lists moderation when at least one account enables it", () => {
    ...
    for (const channelConfig of cases) {
      const cfg = channelConfig as unknown as ApmClawConfig;
      const actions = discordMessageActions.listActions?.({ cfg }) ?? [];
