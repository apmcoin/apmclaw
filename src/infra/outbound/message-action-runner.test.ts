...
describe("runMessageAction sandboxed media validation", () => {
  ...
  it("allows media paths under preferred ApmClaw tmp root", async () => {
    const tmpRoot = resolvePreferredApmClawTmpDir();
    ...
    try {
      ...
      const hostTmpOutsideApmClaw = path.join(os.tmpdir(), "outside-openclaw", "test-media.png");
      await expect(
        runMessageAction({
          cfg: slackConfig,
          action: "send",
