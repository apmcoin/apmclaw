...
export function registerDefaultAuthTokenSuite(): void {
  describe("default auth (token)", () => {
    ...
    test("closes silent handshakes after timeout", async () => {
      vi.useRealTimers();
      const prevHandshakeTimeout = process.env.APMCLAW_TEST_HANDSHAKE_TIMEOUT_MS;
      process.env.APMCLAW_TEST_HANDSHAKE_TIMEOUT_MS = "20";
      try {
        const ws = await openWs(port);
        ...
        if (prevHandshakeTimeout === undefined) {
          delete process.env.APMCLAW_TEST_HANDSHAKE_TIMEOUT_MS;
        } else {
          process.env.APMCLAW_TEST_HANDSHAKE_TIMEOUT_MS = prevHandshakeTimeout;
        }
      }
    });
      ...
      for (const testCase of [
        {
          env: {
            APMCLAW_VERSION: " ",
            APMCLAW_SERVICE_VERSION: "2.4.6-service",
