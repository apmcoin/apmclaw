...
export function createProfileAvailability({
  ...
  const waitForCdpReadyAfterLaunch = async (): Promise<void> => {
    // launchApmClawChrome() can return before Chrome is fully ready to serve /json/version + CDP WS.
  ...
  const ensureBrowserAvailable = async (): Promise<void> => {
    ...
    if (!httpReachable) {
      ...
      const launched = await launchApmClawChrome(current.resolved, profile);
      attachRunning(launched);
      try {
        await waitForCdpReadyAfterLaunch();
      } catch (err) {
        await stopApmClawChrome(launched).catch(() => {});
        setProfileRunning(null);
        throw err;
      }
      return;
    ...
    await stopApmClawChrome(profileState.running);
    setProfileRunning(null);

    const relaunched = await launchApmClawChrome(current.resolved, profile);
