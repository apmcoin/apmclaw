...
import type { ApmClawConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
  validateConfigObjectRawWithPlugins,
  validateConfigObjectWithPlugins,
} from "./validation.js";
import { compareApmClawVersions } from "./version.js";
...
const SHELL_ENV_EXPECTED_KEYS = [
  ...
  "SLACK_BOT_TOKEN",
  "SLACK_APP_TOKEN",
  "APMCLAW_GATEWAY_TOKEN",
  "APMCLAW_GATEWAY_PASSWORD",
];
...
function unsetPathForWrite(
  root: ApmClawConfig,
  pathSegments: string[],
): { changed: boolean; next: ApmClawConfig } {
...
function coerceConfig(value: unknown): ApmClawConfig {
...
function stampConfigVersion(cfg: ApmClawConfig): ApmClawConfig {
...
function warnIfConfigFromFuture(cfg: ApmClawConfig, logger: Pick<typeof console, "warn">): void {
  const touched = cfg.meta?.lastTouchedVersion;
  if (!touched) {
    return;
  }
  const cmp = compareApmClawVersions(VERSION, touched);
  if (cmp === null) {
    return;
  }
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer ApmClaw (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as ApmClawConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): ApmClawConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as ApmClawConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: ApmClawConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as ApmClawConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.APMCLAW_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.APMCLAW_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.APMCLAW_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.APMCLAW_WATCH_SESSION === "string" &&
        deps.env.APMCLAW_WATCH_SESSION.trim().length > 0
          ? deps.env.APMCLAW_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.APMCLAW_WATCH_COMMAND === "string" &&
        deps.env.APMCLAW_WATCH_COMMAND.trim().length > 0
          ? deps.env.APMCLAW_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `APMCLAW_CONFIG_PATH` (and friends) are expected to work even
...
let runtimeConfigSnapshot: ApmClawConfig | null = null;
let runtimeConfigSourceSnapshot: ApmClawConfig | null = null;

function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.APMCLAW_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.APMCLAW_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function getRuntimeConfigSnapshot(): ApmClawConfig | null {
  return runtimeConfigSnapshot;
}

export function loadConfig(): ApmClawConfig {
