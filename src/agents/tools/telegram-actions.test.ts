...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): ApmClawConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as ApmClawConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): ApmClawConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as ApmClawConfig;
    await expect(
...
describe("handleTelegramAction per-account gating", () => {
  ...
  async function expectAccountStickerSend(cfg: ApmClawConfig, accountId = "media") {
    await handleTelegramAction(
