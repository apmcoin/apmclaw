...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: ApmClawConfig },
  primary: string,
) {
...
type SharedDefaultModelCase = {
  apply: (cfg: ApmClawConfig) => { changed: boolean; next: ApmClawConfig };
  defaultModel: string;
  overrideConfig: ApmClawConfig;
  alreadyDefaultConfig: ApmClawConfig;
};
...
describe("applyDefaultModelChoice", () => {
  it("ensures allowlist entry exists when returning an agent override", async () => {
    ...
    const applied = await applyDefaultModelChoice({
      ...
      applyProviderConfig: (config: ApmClawConfig) => config,
      applyDefaultConfig: (config: ApmClawConfig) => config,
      noteAgentModel,
...
describe("shared default model behavior", () => {
  it("sets defaults when model is unset", () => {
    for (const testCase of SHARED_DEFAULT_MODEL_CASES) {
      const cfg: ApmClawConfig = { agents: { defaults: {} } };
