...
const GOOGLE_LIVE_SINGLE_KEY = "APMCLAW_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "APMCLAW_LIVE_ANTHROPIC_KEY",
    listVar: "APMCLAW_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "APMCLAW_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `APMCLAW_LIVE_${base}_KEY`;
