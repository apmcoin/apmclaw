...
describe("doctor command", () => {
  ...
  it("warns when the state directory is missing", async () => {
    ...
    process.env.APMCLAW_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when APMCLAW_GATEWAY_TOKEN is set", async () => {
    mockDoctorConfigSnapshot({
      config: {
        gateway: { mode: "local" },
      },
    ...
    process.env.APMCLAW_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
