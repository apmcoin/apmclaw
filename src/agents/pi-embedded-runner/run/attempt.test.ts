...
function createOllamaProviderConfig(injectNumCtxForOpenAICompat: boolean): ApmClawConfig {
  return {
    models: {
      providers: {
        ollama: {
  ...
  it("includes resolved auth profile fields for context-engine afterTurn compaction", () => {
    const legacy = buildAfterTurnLegacyCompactionParams({
      attempt: {
        ...
        config: { plugins: { slots: { contextEngine: "lossless-claw" } } } as ApmClawConfig,
