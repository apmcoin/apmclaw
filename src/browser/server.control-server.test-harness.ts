...
export function restoreGatewayPortEnv(prevGatewayPort: string | undefined): void {
  if (prevGatewayPort === undefined) {
    delete process.env.APMCLAW_GATEWAY_PORT;
    return;
  }
  process.env.APMCLAW_GATEWAY_PORT = prevGatewayPort;
}
...
vi.mock("./chrome.js", () => ({
  ...
  launchApmClawChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.prevGatewayPort = process.env.APMCLAW_GATEWAY_PORT;
  process.env.APMCLAW_GATEWAY_PORT = String(state.testPort - 2);
  ...
  state.prevGatewayToken = process.env.APMCLAW_GATEWAY_TOKEN;
  state.prevGatewayPassword = process.env.APMCLAW_GATEWAY_PASSWORD;
...
export function restoreGatewayAuthEnv(
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.APMCLAW_GATEWAY_TOKEN = prevGatewayToken;
  }
  if (prevGatewayPassword === undefined) {
    delete process.env.APMCLAW_GATEWAY_PASSWORD;
  } else {
    process.env.APMCLAW_GATEWAY_PASSWORD = prevGatewayPassword;
  }
}
