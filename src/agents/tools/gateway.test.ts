...
describe("gateway tool defaults", () => {
  const envSnapshot = {
    openclaw: process.env.APMCLAW_GATEWAY_TOKEN,
  ...
  afterAll(() => {
    if (envSnapshot.apmclaw === undefined) {
      delete process.env.APMCLAW_GATEWAY_TOKEN;
    } else {
      process.env.APMCLAW_GATEWAY_TOKEN = envSnapshot.apmclaw;
    }
  ...
  it("uses APMCLAW_GATEWAY_TOKEN for allowlisted local overrides", () => {
  ...
  it("does not leak local env/config tokens to remote overrides", () => {
    process.env.APMCLAW_GATEWAY_TOKEN = "local-env-token";
