...
export function isCommandEnabled(cfg: ApmClawConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: ApmClawConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as ApmClawConfig),
...
export function getCommandDetection(_cfg?: ApmClawConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: ApmClawConfig) {
