...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".apmclaw-profile-decorated");
}
 ...
 */
export function decorateApmClawProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_APMCLAW_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_APMCLAW_BROWSER_COLOR).toUpperCase();
