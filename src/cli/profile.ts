...
function resolveProfileStateDir(
  ...
  return path.join(resolveRequiredHomeDir(env as NodeJS.ProcessEnv, homedir), `.apmclaw${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  env.APMCLAW_PROFILE = profile;

  const stateDir = env.APMCLAW_STATE_DIR?.trim() || resolveProfileStateDir(profile, env, homedir);
  if (!env.APMCLAW_STATE_DIR?.trim()) {
    env.APMCLAW_STATE_DIR = stateDir;
  }

  if (!env.APMCLAW_CONFIG_PATH?.trim()) {
    env.APMCLAW_CONFIG_PATH = path.join(stateDir, "apmclaw.json");
  }

  if (profile === "dev" && !env.APMCLAW_GATEWAY_PORT?.trim()) {
    env.APMCLAW_GATEWAY_PORT = "19001";
  }
}
