...
describe("chrome extension relay server", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv([
      ...
      "APMCLAW_EXTENSION_RELAY_RECONNECT_GRACE_MS",
      "APMCLAW_EXTENSION_RELAY_COMMAND_RECONNECT_WAIT_MS",
    ]);
    process.env.APMCLAW_GATEWAY_TOKEN = TEST_GATEWAY_TOKEN;
    delete process.env.APMCLAW_EXTENSION_RELAY_RECONNECT_GRACE_MS;
    delete process.env.APMCLAW_EXTENSION_RELAY_COMMAND_RECONNECT_WAIT_MS;
  });
  ...
  it("closes CDP clients after reconnect grace when extension stays disconnected", async () => {
    process.env.APMCLAW_EXTENSION_RELAY_RECONNECT_GRACE_MS = "150";
  ...
  it("stops advertising websocket endpoint after reconnect grace expires", async () => {
    process.env.APMCLAW_EXTENSION_RELAY_RECONNECT_GRACE_MS = "120";
  ...
  it("reuses an already-bound relay port when another process owns it", async () => {
    ...
    const fakeRelay = createServer((req, res) => {
      if (req.url?.startsWith("/json/version")) {
        ...
        res.end(JSON.stringify({ Browser: "ApmClaw/extension-relay" }));
        return;
      }
  ...
  it(
    ...
    async () => {
      process.env.APMCLAW_EXTENSION_RELAY_RECONNECT_GRACE_MS = "200";
  ...
  it(
    ...
    async () => {
      process.env.APMCLAW_EXTENSION_RELAY_RECONNECT_GRACE_MS = "2000";
